...
class Attention_NodeLevel(nn.Module):
    ...
    def forward(self, input1, input2, adj):
        print('=================')
        print('input1, input2, adj', input1.shape, input2.shape, adj.shape)
        print('self.dim_features', self.dim_features)# 512

        #[30, 512]) torch.Size([30, 512]) torch.Size([30, 30])

        #[30, 512]) torch.Size([30, 512]) torch.Size([30, 9])
        ...
        print('self.a1', self.a1.shape)#self.a1 torch.Size([512, 1])
        e1 = torch.matmul(h, self.a1).repeat(1, M)
        
        print('e1', e1.shape)#[30, 30])
        e2 = torch.matmul(g, self.a2).repeat(1, N).t()
        print('e2', e2.shape)#[30, 30])
        e = e1 + e2  
        e = self.leakyrelu(e)
        ...
        if 'sparse' in adj.type():
            ...
            # adj_dense shape: torch.Size([30, 30])
            # e shape: torch.Size([30, 30])
            # zero_vec shape: torch.Size([30, 30])

            print("adj_dense shape:", adj_dense.shape)#[30, 9])
            print("e shape:", e.shape)#[30, 30])
            print("zero_vec shape:", zero_vec.shape)#[30, 30])
